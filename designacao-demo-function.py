...
def participa(participantes, servidor):


    # Se tiver so um Ativo no Banco de Dados, Participa
    ...
    id_servidor = int(servidor[0])
    peso_servidor = int(servidor[2])
    saldo_servidor = int(servidor[3])
    ...
    for x in participantes:
        ...
        peso_banco = int(x[2])
        saldo_banco = int(x[3])
        diferenca = int(saldo_servidor - saldo_banco)

        # Se Servidor do Banco for Diferente do Atual e Tiverem o Mesmo Peso
        if (id_servidor != id_banco and peso_servidor == peso_banco):

            # Se a Diferença for Menor que o Intervalo
            if (diferenca < intervalo and saldo_banco < int(peso_servidor * 2)):
                sim_participa += 1
                print(str(x[1]) + ' Participa')
            else:
                print(str(x[1]) + ' Não Participa')
                nao_participa += 1
    ...
    if (sim_participa > nao_participa):
        return True
    else:
        return False
